...
present in the sample as well as their abundances (see
[Example Data](#example-data) for more information about output data).
To address the protein inference, most of them
combine the set of all plausible protein identifiers matching a common set of
peptides into a protein group.

Some multiplex proteomics technologies allow to process many experimental
samples simultaneously. For example, using iTRAQ we can simultaneously identify
and quantitate proteins present in eight samples. However, moderate to large
studies require the analysis of more than eight samples thus requiring multiple
batches or experimental runs to quantitate the proteins in all available
samples. The protein groups, however, are created for each experimental run
...
(i.e., global groups) are called "connected" groups.
...
protein identifiers from gene PRSS2 (UniPort was not used).
In *BET2007_v339* the
